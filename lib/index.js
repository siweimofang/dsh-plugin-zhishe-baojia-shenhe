 ...
 *       v0.6.1: vision_structured 已切默认true(0905批次六对拍: structured召回
 *       DS 84.6%/BL 87.2% ≥ text 82.0%/85.9%, 同分器头对头), 传false退回文本正则路径;
    ...
    tools: [
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            vision_structured: {
              type: "boolean",
              description: "v0.6.1起默认true: 结构化视觉提取(视觉模型直接输出JSON条目,单引擎比价更稳;失败自动回退文本路径)。传false退回文本正则路径。",
            },
          },
          required: [],
        },
      },
      {
        ...
        parameters: {
          ...
          properties: {
            ...
            vision_structured: {
              type: "boolean",
              description: "v0.6.1起默认true: 结构化视觉提取(失败自动回退文本路径)。传false退回文本正则路径。",
            },
          },
          required: ["images"],
        },
      ...
      {
        name: "hetong_shenhe",
        description: "装修合同审核:上传合同条款页截图(必须是脱敏件!)自动提取关键条款(付款比例/增项上限/工期/保修期/违约责任),按行业惯例与法定底线(保修2年/防水5年)审核,输出风险清单+签约避坑建议。红线:合同原件属红档禁止上传,只传涂掉姓名/电话/证件号的脱敏件;检出未脱敏联系方式会自动打码并红档警告。COMPLIANCE_MODE=strict时合规硬路由生效:strict模式下未脱敏件将被拒收——合同图片必须走百炼合规路线(未配置百炼时拒收并引导改传彻底脱敏件或contract_text文本,不经视觉云)。",
        parameters: {
          type: "object",
          properties: {
            contract_images: {
...
export function apply() {
  return {
    ...
    baojia_shenhe: async (params) => {
      const { quote_text, quote_image, city = '沈阳', tier = '中端', area = null, vision_structured = true } = params;

      // 图片输入 → 视觉提取(v0.6.1: 结构化优先为默认, 传vision_structured=false或失败时回退文本路径)
    ...
    baojia_image_audit: async (params) => {
      const { images, city = '沈阳', tier = '中端', area = null, vision_structured = true } = params;

      if (!images || !Array.isArray(images) || images.length === 0) {
        return { status: "error", message: "请提供至少一张报价单图片" };
      }

      // v0.6.1: 结构化优先为默认, 传vision_structured=false或失败时回退文本路径
      let finalItems = null;
      let structuredFallback = null;
      let ocrResult = null;
      ...
      try {
        if (contract_images && Array.isArray(contract_images) && contract_images.length > 0) {
          ...
          meta = {
            ...
            ...(ocr.compliance_rerouted && { compliance_rerouted: ocr.compliance_rerouted }),
            ...(ocr.compliance_blocked && { compliance_blocked: ocr.compliance_blocked }),
          };
          if (!ocr.success) {
            // v0.6.0 合规预检拒收(spec §6-3): 透出 compliance_refused, 文案加拒收头便于调用方识别
            if (ocr.compliance_refused) {
              return {
                status: 'error',
                compliance_refused: true,
                message: `【合规硬路由-已拒收】${ocr.error}`,
                pii: ocr.pii,
              };
            }
        ...
        let formatted = formatContractReport(clauses, audit, tips, meta);
        // v0.6.0 报告装配层(spec §6-2 拍板③=B): 顶部横幅——封存警告置顶, 自动改道走提示行
        if (meta && meta.compliance_blocked) {
          formatted = `⛔ 红档封存警告: ${meta.compliance_blocked}\n\n` + formatted;
        } else if (meta && meta.compliance_rerouted) {
          formatted = `ℹ️ ${meta.compliance_rerouted}\n\n` + formatted;
        }
