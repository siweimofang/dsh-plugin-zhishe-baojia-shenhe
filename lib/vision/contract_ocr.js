/**
 * 合同条款OCR模块 - v0.5.0: 双路线(DeepSeek/百炼)+PII合规钩子 (P2 Step2)
 *
 ...
 * 注: PII 打码是过渡护栏; v0.5.0 检出PII且供应商非百炼时附 compliance_warning(提示级),
...
export async function extractContractFromImages(imageInputs, options = {}) {
    ...
    const pages = [];
    let usage = null;
    let providerUsed = null; // v0.5.0: 记录实际供应商,供PII合规提示
    ...
    // v0.5.0 合规钩子(提示级): 检出PII且供应商非百炼 → 旗标;硬路由待spec定稿
    const compliance_warning = piiFound.length > 0 && providerUsed !== 'bailian'
        ? `检出个人信息(${piiFound.join('/')})且当前视觉供应商为「${providerUsed || 'unknown'}」非百炼——按红档红线建议敏感件改走百炼路线或先彻底脱敏(spec定稿后升级为硬拒收)`
        : null;
    return {
        success: true,
