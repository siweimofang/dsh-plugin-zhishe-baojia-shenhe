...
export function auditContract(clauses, options = {}) {
    ...
    if (termsAll.length >= 2) {
        ...
        if (condKnown.length > 0 && !condKnown.some(x => /验收|合格|竣工|开工|进场|隐蔽|泥木|油漆|水电/.test(x))) {
            add('payment_progress', 'SEV_MEDIUM', '付款未挂钩工程进度',
                '付款节点未见与工程进度挂钩(未检出验收/竣工等进度条件)——建议按节点付款(开工→水电验收→泥木验收→竣工验收),纯按日期付款对业主无保护。');
        }
    }

    // 12. 日违约金过高(行业惯例 0.05%~0.1%/日)
    if (c.delay_penalty_daily_percent != null && c.delay_penalty_daily_percent > 0.5) {
        add('penalty_ratio', 'SEV_MEDIUM', '日违约金偏高',
            `工期延误违约金 ${c.delay_penalty_daily_percent}%/日,明显高于行业惯例(0.05%~0.1%/日)——双向条款下过高一方也可主张法院调低,建议回归惯例区间。`);
    }
